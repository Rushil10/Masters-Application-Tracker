...
function AddLor({navigation}) {
  ...
  const pickResume = async () => {
    ...
    if (res.size / 1000000 > 1) {
      showErrorModal('Maximum Size of PDF must be below 1mb');
      return;
    }
    setFileData(res);
  };
  ...
  const addToMongoDb = async (name, link) => {
    ...
    //console.log(body);
    var res = await axios.post(`${url}/lor/add`, body, {
      headers: {
        authorization: `Bearer ${token}`,
      },
    });
    if (!res.data) {
      showErrorModal('Something went wrong while storing your LOR.');
    } else {
